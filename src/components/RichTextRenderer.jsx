...
function YouTubeVideo({ entry }) {
  const { title, videoId, description, autoplay, showControls, startTime } =
    entry.fields;
  ...
  if (showControls === false) params.append("controls", "0");
  ...
  return (
    ...
    >
      <iframe
        ...
        aria-label={`YouTube video: ${title || "Embedded video"}`}
      />
      {description && (
        <p
          style={{
...
const options = {
  renderMark: {
    ...
    [MARKS.UNDERLINE]: (text) => (
      <u style={{ textDecoration: "underline" }}>{text}</u>
    ),
    [MARKS.CODE]: (text) => (
      <code
        style={{
  ...
  renderNode: {
    ...
    [BLOCKS.EMBEDDED_ENTRY]: (node) => {
      ...
      if (entry.sys.contentType?.sys?.id === "youtubeVideo") {
        return <YouTubeVideo entry={entry} />;
      }
      ...
      return (
        ...
        >
          Embedded content: {entry.fields?.title || "Unknown content"}
        </div>
      );
    },
  },
