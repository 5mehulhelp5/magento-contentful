/**
 * Unified Recipe Card Template System
...
/**
 * Generate recipe card content (without the outer <a> wrapper)
 * Useful for cases where the link wrapper is handled separately
 * @param {Object} recipe - Contentful recipe object
 * @param {string} linkBase - Base URL for recipe links
 * @returns {string} HTML string for recipe card content
 */
function generateRecipeCardContent(recipe, linkBase = "/preview/recipe") {
  const fullHTML = generateRecipeCardHTML(recipe, linkBase);
...
/**
 * Extract recipe metadata for additional processing
 * @param {Object} recipe - Contentful recipe object
 * @param {string} linkBase - Base URL for recipe links
 * @returns {Object} Processed recipe metadata
 */
function getRecipeMetadata(recipe, linkBase = "/preview/recipe") {
  const { sys, fields = {} } = recipe || {};
  const {
    title,
    body,
